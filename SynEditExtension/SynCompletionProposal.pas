...
type
  ...
  private
    ...
    FCurrentEditor: TCustomSqlEditor;
    FTitle: string;
    FTitleFont: TFont;
    FFont: TFont;
    FResizeable: Boolean;
...
{$ELSE}
    procedure WMMouseWheel(var Msg: TMessage); message LM_MOUSEWHEEL;
...
{$ENDIF}
  public
    ...
    function LogicalToPhysicalIndex(Index: NativeInt): NativeInt;
    function PhysicalToLogicalIndex(Index: NativeInt): NativeInt;
    ...
    property CurrentEditor  : TCustomSqlEditor read fCurrentEditor write fCurrentEditor;
  ...
  private
    ...
    function GetEditor(i: Integer): TCustomSqlEditor;
    procedure InternalCancelCompletion; //GBN 25/02/2002
  protected
    procedure DoExecute(AEditor: TCustomSqlEditor); virtual;
    procedure Notification(AComponent: TComponent; Operation: TOperation); override;
  ...
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure AddEditor(AEditor: TCustomSqlEditor);
    function RemoveEditor(AEditor: TCustomSqlEditor): boolean;
    ...
    procedure ActivateTimer(ACurrentEditor: TCustomSqlEditor);
    procedure DeactivateTimer;
    property Editors[i: Integer]: TCustomSqlEditor read GetEditor;
    property CompletionStart: Integer read FCompletionStart write FCompletionStart; // ET 04/02/2003
  published
    property ShortCut: TShortCut read FShortCut write SetShortCut;
    property Editor: TCustomSqlEditor read FEditor write SetEditor;
  ...
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure Execute(token: string; Editor: TCustomSqlEditor);
    procedure ExecuteEx(token: String; Editor: TCustomSqlEditor; LookupIfNotExact: Boolean);
...
begin
  Visible := True;
  if DisplayType = ctCode then
    (CurrentEditor as TCustomSqlEditor).AddFocusControl(Self);
end;

procedure TSynBaseCompletionProposalForm.Deactivate;
begin
  if (DisplayType = ctCode) then
    (CurrentEditor as TCustomSqlEditor).RemoveFocusControl(Self);
  Visible := False;
end;
...
function TSynBaseCompletionProposalForm.LogicalToPhysicalIndex(Index: NativeInt): NativeInt;
begin
  if FMatchText and (Index >= 0) and (Index < FAssignedList.Count) then
    Result := NativeUInt(FAssignedList.Objects[Index])
  else
    Result := Index;
end;

function TSynBaseCompletionProposalForm.PhysicalToLogicalIndex(Index: NativeInt): NativeInt;
var
  i : Integer;
begin
  if FMatchText then
  begin
    Result := -1;
    for i := 0 to FAssignedList.Count - 1 do
      if NativeUInt(FAssignedList.Objects[i]) = Index then
      begin
        Result := i;
        break;
      end;
...
{$ENDIF}
  ...
  Inc(fMouseWheelAccumulator, SmallInt(Msg.wParam));
...
procedure TSynBaseCompletionProposal.HookedEditorCommand(Sender: TObject;
  ...
  var AChar: TUTF8Char; Data, HandlerData: Pointer);
begin
  // Do nothing here, used in TSynCompletionProposal
end;
...
begin
  ...
  if Assigned(F.CurrentEditor) then
    with F.CurrentEditor as TCustomSQLEditor do
    begin
      ...
      try
        if FAdjustCompletionStart then
          FCompletionStart := BufferCoord(FCompletionStart, CaretY).X;
        BlockBegin := BufferCoord(FCompletionStart, CaretY);
        if EndToken = #0 then
          BlockEnd := BufferCoord(WordEnd.X, CaretY)
        else
        ...
        begin
          ...
          begin
            if (Form.FAssignedList.Count > Position) then
              ...
              if (InsertList.Count > NativeUInt(Form.FAssignedList.Objects[position])) and
                 ((scoEndCharCompletion in fOptions) or (EndToken = #0)) then
                Value := InsertList[NativeUInt(Form.FAssignedList.Objects[position])]
              else
                Value := SelText
            else
              Value := SelText;
    ...
    if (System.Pos(Key, TriggerChars) > 0) and not F.Visible then
      ...
      //GBN 18/02/2002
        if (Sender is TCustomSynEdit) then
          DoExecute(Sender as TCustomSqlEditor)
        else
          if Assigned(Form.CurrentEditor) then
            DoExecute(Form.CurrentEditor as TCustomSqlEditor);
      end;

  end;
end;

procedure TSynCompletionProposal.SetEditor(const Value: TCustomSqlEditor);
begin
...
begin
  ...
  begin
    if Editor = AComponent then
      Editor := nil
    else if AComponent is TCustomSqlEditor then
      RemoveEditor( TCustomSqlEditor(AComponent) );
  end;
...
begin
  ...
  begin
    ...
    begin
      Form.CurrentEditor := Sender as TCustomSqlEditor;
      Key := 0;
      DoExecute(Sender as TCustomSqlEditor);
    end;
...
begin
  ...
  begin
    if (Pos(Key, TriggerChars) <> 0) then
      ActivateTimer(Sender as TCustomSqlEditor)
    else
      DeactivateTimer;
...
procedure TSynCompletionProposal.ActivateTimer(ACurrentEditor: TCustomSqlEditor);
begin
  if Assigned(FTimer) then
  begin
...
begin
  ...
  while fEditors.Count <> 0 do
    RemoveEditor(TCustomSqlEditor(FEditors.Last));
...
procedure TSynCompletionProposal.AddEditor(AEditor: TCustomSqlEditor);
var
  i : integer;
begin
...
function TSynCompletionProposal.GetEditor(i: integer): TCustomSqlEditor;
begin
  if (i < 0) or (i >= EditorsCount) then
    Result := nil
  else
    Result := TCustomSqlEditor(fEditors[i]);
end;

function TSynCompletionProposal.RemoveEditor(AEditor: TCustomSqlEditor): boolean;
var
  i: integer;
begin
...
procedure TSynCompletionProposal.DoExecute(AEditor: TCustomSqlEditor);
  ...
  procedure CopyWordBreakCharsToCharSet(const AEditor: TCustomSqlEditor;
  ...
  if i <> -1 then
    ...
    begin
      ...
      begin
        ...
        begin
          p := ClientToScreen( RowColumnToPixels(TPoint(DisplayXY)) );
          Inc(p.y, LineHeight);
        end;
        ...
        if scoConsiderWordBreakChars in Self.Options then
          CopyWordBreakCharsToCharSet(Form.CurrentEditor,Form.FWordBreakChars);

        CopyStringToCharSet(EndOfTokenChr, Form.FWordBreakChars);

        FPreviousToken := GetPreviousToken(Form.CurrentEditor as TCustomSqlEditor);
...
{                if Pos(AChar, FTriggerChars) > 0 then
            ...
            else DoExecute(Sender as TCustomSqlEditor);
            end;
          end;
        else DoExecute(Sender as TCustomSqlEditor);
        end;
...
begin
  ...
  begin
    Execute(GetPreviousToken(Sender as TCustomSqlEditor), Sender as TCustomSqlEditor);
    fNoNextKey := true;
    Key := 0;
  end;
end;
...
procedure TSynAutoComplete.Execute(token: string; Editor: TCustomSqlEditor);
begin
  ExecuteEx(token, Editor, FDoLookup);
end;

procedure TSynAutoComplete.ExecuteEx(token: String; Editor: TCustomSqlEditor;
...
//      Editor.UndoList.AddChange(crAutoCompleteBegin, StartOfBlock, StartOfBlock, '', smNormal);
      ...
      BeginningSpaceCount := Editor.DisplayXY.Column - 1;  //GBN 2002/04/24
...
procedure TSynAutoComplete.SetEditor(const Value: TCustomSqlEditor);
begin
